...
<head>
  ...
  <title>SKATEBOARDS AND ROLLERBLADES : Orders</title>
  <!-- Bootstrap -->
  ...
  <div class="row">
    ...
    <form action="orders.php" method="post" class="form-horizontal">
      ...
      <div class="form-group">
          ...
          <div class="col-sm-9">
      <input name="oid" type="text" class="form-control" id="orderid" placeholder="Auto generate Order ID"  readonly value="<?php if(isset($_GET['edit'])) echo $editrow['fld_order_num']; ?>" required>
    </div>
  </div>

      <div class="form-group">
          <label for="orderdates" class="col-sm-3 control-label">Date</label>
          <div class="col-sm-9">
      <input name="orderdate" class="form-control" id="orderdates" placeholder="Auto generate Order Date" type="text" readonly value="<?php if(isset($_GET['edit'])) echo $editrow['fld_order_date']; ?>" required>
    </div>
  </div>
      ...
      try {
        ...
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
          $stmt = $conn->prepare("SELECT * FROM tbl_staffs_a165959_pt2");
        $stmt->execute();
      ...
      try {
        $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
          $stmt = $conn->prepare("SELECT * FROM tbl_customers_a165959_pt2");
        $stmt->execute();
        $result = $stmt->fetchAll();
      }
      catch(PDOException $e){
    ...
    <div class="col-xs-12 col-sm-10 col-sm-offset-1 col-md-8 col-md-offset-2">
      ...
      try {
        $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $sql = "SELECT * FROM tbl_orders_a165959, tbl_staffs_a165959_pt2, tbl_customers_a165959_pt2 WHERE ";
        $sql = $sql."tbl_orders_a165959.fld_staff_num = tbl_staffs_a165959_pt2.fld_staff_id and ";
        $sql = $sql."tbl_orders_a165959.fld_customer_num = tbl_customers_a165959_pt2.fld_customer_id";
    ...
    <div class="col-xs-12 col-sm-10 col-sm-offset-1 col-md-8 col-md-offset-2">
      <nav>
          ...
          try {
            $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
            $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stmt = $conn->prepare("SELECT * FROM tbl_orders_a165959");
            $stmt->execute();
            $result = $stmt->fetchAll();
